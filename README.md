...
- [x] Phase 5B: Canonical Vector Lab branding sync. Family mark and per-tool icon migrated to the standardised `public/icons/vector-lab-*.svg` paths now shared with Manifold Atlas and Diffusion Atlas. The family mark is the dark-navy (`#0a1f44`) "VL" polygon letterform; the Theoryscope tool icon (point cloud + two eigenvector axes through the centroid) keeps its inner-tier gold (`#b8941e`). Old `public/icon.svg` and `public/vector-lab-mark.svg` removed.
